...
def inject_css():
    ...
    st.markdown(
        ...
        .pill .dot {
            ...
            border-radius: 50%;
            background: var(--o-2);
            box-shadow: 0 0 0 3px rgba(255, 154, 86, 0.20);
        }

        .pill.live .dot {
            background: #55c98a;
            box-shadow: 0 0 0 3px rgba(85, 201, 138, 0.20);
        ...
        .badge.local {
            ...
            border: 1px solid rgba(255, 154, 86, 0.30);
        }

        .badge.remote {
            color: #8a4a17;
            background: rgba(255, 204, 150, 0.35);
            border: 1px solid rgba(217, 90, 42, 0.30);
        }
        ...
        .server-tool {
            ...
            border: 1px solid rgba(245, 220, 200, 0.75);
            border-radius: 9px;
        ...
        .server-tool .dot {
            ...
            border-radius: 50%;
            background: var(--o-2);
            box-shadow: 0 0 0 2px rgba(255, 154, 86, 0.18);
            flex: none;
        }
        ...
        .tool-event-badge.ok {
            color: #1f7a4d;
            background: rgba(85, 201, 138, 0.14);
        ...
        .tool-event-badge.fail {
            ...
            border: 1px solid rgba(217, 90, 42, 0.32);
        }
        ...
        .stButton > button:hover {
            ...
            color: #fff !important;
            border-color: var(--o-3) !important;
            transform: translateY(-2px);
        ...
        [data-testid="stTextInput"] input:focus {
            border-color: var(--o-2) !important;
...
st.markdown(
    ...
    '<div class="pill-row">'
    '<span class="pill live"><span class="dot"></span>Ollama</span>'
    '<span class="pill live"><span class="dot"></span>MCP Client</span>'
    '<span class="pill"><span class="dot"></span>Qwen2.5:3b</span>'
    '<span class="pill"><span class="dot"></span>Local + Remote MCP</span>'
    '</div>'
...
def initialize():
    ...
    with st.spinner("Connecting to MCP servers..."):
        ...
        try:
            ...
            st.session_state.tools_by_server = result["tools_by_server"]
            st.session_state.tools_by_name = result["tools_by_name"]
            st.session_state.failed = result["failed"]
            st.session_state.skipped = result["skipped"]
...
with st.sidebar:
    ...
    st.markdown(
        '<div class="side-title">MCP Network</div>'
        '<div class="side-sub">Connected servers and tools</div>',
        unsafe_allow_html=True,
    )

    tools_by_server = st.session_state.get(
        "tools_by_server", {}
    )

    if not tools_by_server:
        st.warning("No MCP tools loaded.")
    ...
    for server_name, tools in tools_by_server.items():
        ...
        st.markdown(
            ...
            f'<div class="server-name">{server_name}</div>'
            f'<span class="badge {badge_class}">{scope}</span>'
            f'</div>'
    ...
    for server, error in st.session_state.get(
        "failed", {}
    ).items():
        st.error(f"{server}\n\n{error}")

    for server, reason in st.session_state.get(
        "skipped", {}
    ).items():
        st.warning(f"{server}\n\n{reason}")

    for warning in st.session_state.get(
        "warnings", []
    ):
        st.info(warning)
    ...
    if st.button("Reload MCP Servers", use_container_width=True):
    ...
    for event in events:
        ...
        with st.expander(
            ...
            st.markdown(
                ...
                f'<span class="tool-event-name">{name}</span>'
                f'<span class="tool-event-badge {status_class}">'
                f'{icon} {"success" if ok else "error"}'
                f'</span>'
                f'<span class="tool-event-badge loc">{location}</span>'
                f'</div>'
            ...
            st.markdown(
                '<div class="tool-event-label">Arguments</div>',
                unsafe_allow_html=True,
            )
            ...
            st.markdown(
                '<div class="tool-event-label">Tool output</div>',
                unsafe_allow_html=True,
            )
...
for item in st.session_state.get("transcript", []):
    ...
    with st.chat_message(item["role"]):
        ...
        render_tool_events(item.get("tools", []))
...
if user_text:
    ...
    with st.chat_message("assistant"):
        ...
        render_tool_events(events)

        if error:
            st.error(answer)
        else:
            st.markdown(
                answer or "_No response returned._"
            )
