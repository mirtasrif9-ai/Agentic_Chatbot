...
SERVERS = { 
    "Demo Server": {
            ...
            "args": [
                ...
                "fastmcp",
                "run",
                "/Users/jishnu/Desktop/expense-tracker-mcp-server/main.py"
           ]
        },
         "manim-server": {
                "transport": "stdio",
                ...
                "args": [
                "/Users/jishnu/desktop/manim-mcp-server/src/manim_server.py"
              ],
                "env": {
        ...
        "url": "https://shaky-teal-porcupine.fastmcp.app/mcp"
       }
}
...
if "initialized" not in st.session_state:
    # Persistent event loop: reused across reruns so cached async clients
    # (ChatOpenAI, MCP client) stay bound to a loop that never closes.
    ...
    tools = st.session_state.loop.run_until_complete(st.session_state.client.get_tools())
...
# Run the whole turn (first pass, tool calls, final pass) on a single event loop
async def run_turn():
...
if user_text:
    ...
    first, final = st.session_state.loop.run_until_complete(run_turn())
